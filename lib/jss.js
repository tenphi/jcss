...
jss.render = function (css, minified, namespace, offset) {
    var decoration = {
        min: {
            open: '{',
            nl: '',
...
jss.normalize = function(styles) {
    var newstyles, selector, sel, options, style, style2, name, i, temp, temp2, fselector;
	for (selector in styles) {
		...
		for (style in styles[selector]) {
			if (this.handlers[style] instanceof Function) {
				...
				for (i = 0; i < options.length; i++) {
					newstyles = this.handlers[style](options[i]);
...
jss.handler = function (name, handler) {
	if (!handler) {
...
jss.join = function (styles, prefixes) {
